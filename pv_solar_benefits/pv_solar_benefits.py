...
def get_pv_solar_benefits(roof_information: Dict) -> Dict:
    ...
    roof_information = roof_information["data"]
    ...
    url = "https://developer.nrel.gov/api/pvwatts/v5.json?api_key=q8VOaHxmP5GTjliWsOuyC6KLoU7f0SI6wYDBxvPt&lat=%s&lon=%s&system_capacity=%.2f&azimuth=%s&tilt=%s&array_type=1&module_type=1&losses=10&dataset=intl" % (int(lat), int(lon), round(system_capacity,2), azimuth, TILT)
    ...
    try:
        ...
        solar_benefits = {"data": {
                   ...
                   "annualProduction": "no data available",
